...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private BTCRegime[] cacheBTCRegime;
		public BTCRegime BTCRegime(int period, double choppinessRangeThreshold, double choppinessTrendThreshold, double aDXTrendThreshold, double aDXRangeThreshold, bool showRegimeLabel, bool colorBackground)
		{
			return BTCRegime(Input, period, choppinessRangeThreshold, choppinessTrendThreshold, aDXTrendThreshold, aDXRangeThreshold, showRegimeLabel, colorBackground);
		}

		public BTCRegime BTCRegime(ISeries<double> input, int period, double choppinessRangeThreshold, double choppinessTrendThreshold, double aDXTrendThreshold, double aDXRangeThreshold, bool showRegimeLabel, bool colorBackground)
		{
			if (cacheBTCRegime != null)
				for (int idx = 0; idx < cacheBTCRegime.Length; idx++)
					if (cacheBTCRegime[idx] != null && cacheBTCRegime[idx].Period == period && cacheBTCRegime[idx].ChoppinessRangeThreshold == choppinessRangeThreshold && cacheBTCRegime[idx].ChoppinessTrendThreshold == choppinessTrendThreshold && cacheBTCRegime[idx].ADXTrendThreshold == aDXTrendThreshold && cacheBTCRegime[idx].ADXRangeThreshold == aDXRangeThreshold && cacheBTCRegime[idx].ShowRegimeLabel == showRegimeLabel && cacheBTCRegime[idx].ColorBackground == colorBackground && cacheBTCRegime[idx].EqualsInput(input))
						return cacheBTCRegime[idx];
			return CacheIndicator<BTCRegime>(new BTCRegime(){ Period = period, ChoppinessRangeThreshold = choppinessRangeThreshold, ChoppinessTrendThreshold = choppinessTrendThreshold, ADXTrendThreshold = aDXTrendThreshold, ADXRangeThreshold = aDXRangeThreshold, ShowRegimeLabel = showRegimeLabel, ColorBackground = colorBackground }, input, ref cacheBTCRegime);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.BTCRegime BTCRegime(int period, double choppinessRangeThreshold, double choppinessTrendThreshold, double aDXTrendThreshold, double aDXRangeThreshold, bool showRegimeLabel, bool colorBackground)
		{
			return indicator.BTCRegime(Input, period, choppinessRangeThreshold, choppinessTrendThreshold, aDXTrendThreshold, aDXRangeThreshold, showRegimeLabel, colorBackground);
		}

		public Indicators.BTCRegime BTCRegime(ISeries<double> input , int period, double choppinessRangeThreshold, double choppinessTrendThreshold, double aDXTrendThreshold, double aDXRangeThreshold, bool showRegimeLabel, bool colorBackground)
		{
			return indicator.BTCRegime(input, period, choppinessRangeThreshold, choppinessTrendThreshold, aDXTrendThreshold, aDXRangeThreshold, showRegimeLabel, colorBackground);
		}
	}
